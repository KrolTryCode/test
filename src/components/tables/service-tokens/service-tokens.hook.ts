...
import { getNodeTokensQueryOptions } from '~/api/queries/token/get-project-node-tokens.query';
...
export const useServiceTokensTable = (nodeId: string) => {
  ...
  const { data: tokens = [], isLoading: isTokenListLoading } = useQuery(
    getNodeTokensQueryOptions(nodeId),
  );
