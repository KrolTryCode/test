...
Убедитесь, что у вас установлен [NodeJS](https://nodejs.org/en) (см. актуальную версию в [.nvmrc](./.nvmrc)),
...
- моковые серверы создаются в `api/mocks/<controller>/controller.ts`, там же описывается словарь используемых URL. Типы описываются в соседнем файле `types.ts`
- `queryFn/mutationFn` содержит вызов настоящего `ApiClient[Secured]` через `.request<T>({ ..., path: <импорт URL из api/mocks/<controller>/controller.ts>})`
- при этом получается, что если ни один файл не импортирует моковый URL, то файл не участвует в финальном бандле и моковый сервер не создается
